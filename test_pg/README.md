...
This folder is the OpenCLIP + pgvector pipeline. Shared code stays in `utils/` and `utils_db/` at the **repo root**. Run the commands below from the repo root unless noted.
...
- **Open a terminal** in the **repo root** (parent of this folder).
- **Run:**
...
- **Output:** `test_pg/train_official.parquet`
...
python test_pg/setup_pgvector.py
```
...
From a terminal you can load the shared parquet in one step: `python test_pg/setup_pgvector.py`.
...
From the repo root, with Docker already running and the catalog imported:

```bash
python test_pg/predict_labeled_image.py --image test_pg/L001_test_tubes__L001_TEST_TUBE_L001_04.jpg --label test_pg/L001_test_tubes__L001_TEST_TUBE_L001_04.txt --output test_pg/overlay.png
```
...
```bash
python test_pg/predict_labeled_image.py --image test_pg/L001_test_tubes__L001_TEST_TUBE_L001_04.jpg --label test_pg/L001_test_tubes__L001_TEST_TUBE_L001_04.txt --yaml path/to/data.yaml --min-n 3 --no-show
```
