...
Cohesion: 0.12
Nodes (31): colorsys, matplotlib, matplotlib_patches, crop_labeled_objects(), load_image_and_label(), Image, Crop each labeled object from ``image`` and attach bbox metadata., Load one image and its YOLO label. Specify the file with ``image_path=``, or an… (+23 more)
...
### Community 9 - "face.py"
Cohesion: 0.05
Nodes (41): cv2, _align_template(), choose_staff_match(), crop_from_box(), draw_landmarks(), encode_crop(), ensure_face_model(), face_in_region() (+33 more)
...
Cohesion: 0.09
Nodes (42): dataclasses, choose_folder(), import_train_catalog(), inventory_name_from_filename(), masked_crops(), _polygon_rows(), Image, ndarray (+34 more)
...
### Community 52 - "dataset.py"
Cohesion: 0.20
Nodes (14): collections_abc, main(), Export the full train image catalog to train_official.parquet (no embeddings)., tqdm, build_embedding_dataframe(), _class_key(), _class_names_for_image(), DataFrame (+6 more)
...
- **Why does `bbox_xyxy()` connect `PETROSAINS inventory similarity (`test_pg`)` to `visualize.py`, `dataset.py`?**
