...
const objectRecognitionGateText = document.getElementById("objectRecognitionGateText")
    || (objectRecognitionGate ? objectRecognitionGate.querySelector("span") : null);
const captureAlert = document.getElementById("captureAlert");
const captureAlertText = document.getElementById("captureAlertText");
const captureAlertClose = document.getElementById("captureAlertClose");
const captureAlertIconOk = document.getElementById("captureAlertIconOk");
const captureAlertIconFail = document.getElementById("captureAlertIconFail");
let captureAlertTimer = null;
let lastTriggerState = "WAITING";
let lastCaptureToastKey = "";
let flowArmed = false;
...
const RECOGNIZE_COVERAGE = 0.70;
/** Face centroid must sit inside this fraction of the gate oval (middle zone). */
...
function faceCenteredInGate(landmarks) {
    const center = faceCentroid(landmarks);
    if (!center) return false;
    const dx = (center.x - GATE_OVAL.cx) / (GATE_OVAL.rx * CENTER_FRAC);
    const dy = (center.y - GATE_OVAL.cy) / (GATE_OVAL.ry * CENTER_FRAC);
    return dx * dx + dy * dy <= 1;
}
...
function faceReady(landmarks) {
    ...
    // Register + recognize both require face centered in the outline middle.
    if (!faceCenteredInGate(landmarks)) return false;
    if (faceUiMode === "register") return coverage >= COVERAGE_READY;
    const span = faceSpan(landmarks);
    return (
        coverage >= RECOGNIZE_COVERAGE
        && span.w >= 0.08
        && span.h >= 0.1
    );
}
...
function updateCoverageLabel(ratio) {
    ...
    el.textContent = `${pct}%`;
    const need = faceUiMode === "register" ? COVERAGE_READY : RECOGNIZE_COVERAGE;
    el.classList.toggle("is-ready", ratio >= need);
}
...
function applyLocalFace(ready, detected, coverage, landmarks = null) {
    ...
    else if (landmarks && !faceCenteredInGate(landmarks)) {
        setFaceHint("Center your face in the outline");
    } else if (
        (coverage || 0) < (faceUiMode === "register" ? COVERAGE_READY : RECOGNIZE_COVERAGE)
    ) {
        setFaceHint(`Fill the outline · ${Math.round((coverage || 0) * 100)}%`);
    } else {
        setFaceHint(
            faceUiMode === "register"
                ? "Hold still — ready to register"
                : "Hold still in the middle of the outline"
        );
    }
}
...
function runBrowserFaceLoop() {
    ...
    try {
        ...
        if (landmarks && landmarks.length) {
            ...
            const staffLocked = Boolean(currentRecognizedStaffName);
            if (staffLocked) {
                landmarkCtx.clearRect(0, 0, landmarkLayer.width, landmarkLayer.height);
            } else if (coverage >= 0.4 || ready) {
                drawMediaPipeLandmarks(landmarks);
            } else {
                landmarkCtx.clearRect(0, 0, landmarkLayer.width, landmarkLayer.height);
            }
            applyLocalFace(ready, true, coverage, landmarks);
            if (ready && faceUiMode === "recognize" && !staffLocked) {
            ...
            } else if (ready && !capturedForModal && !staffLocked) {
...
function enableCaptureFallback() {
    ...
    cameraFacing = preferredCameraFacing();
    liveVideo.classList.add("is-hidden");
    if (unlock) {
        unlock.textContent = captureButtonLabel();
        unlock.classList.remove("is-hidden");
    }
    if (view) {
        view.classList.toggle("is-rear", cameraFacing === "environment");
        view.classList.toggle("has-snapshot", snapshotReady);
    }
...
function applyCapturedPhoto(file) {
    ...
    img.onload = () => {
        ...
        liveVideo.classList.add("is-hidden");
        const view = document.querySelector(".camera-view");
        if (view) view.classList.add("has-snapshot");
...
function openNativeCamera() {
    const input = preferredCameraFacing() === "environment"
...
function showCaptureAlert(ok, message) {
    if (!captureAlert || !captureAlertText) {
        window.alert(message);
        return;
    }
    captureAlert.classList.remove("is-hidden", "is-success", "is-fail");
    captureAlert.classList.add(ok ? "is-success" : "is-fail");
    captureAlertText.textContent = message;
    if (captureAlertIconOk) captureAlertIconOk.classList.toggle("is-hidden", !ok);
    if (captureAlertIconFail) captureAlertIconFail.classList.toggle("is-hidden", ok);
    if (captureAlertTimer) clearTimeout(captureAlertTimer);
    captureAlertTimer = window.setTimeout(() => {
        captureAlert.classList.add("is-hidden");
    }, 4200);
}

function hideCaptureAlert() {
    if (captureAlert) captureAlert.classList.add("is-hidden");
    if (captureAlertTimer) {
        clearTimeout(captureAlertTimer);
        captureAlertTimer = null;
    }
}

function applyObjectRecognitionState(canArm, armed, previewActive = false, state = "") {
    const normalized = String(state || lastTriggerState || "").toUpperCase();
    lastTriggerState = normalized || lastTriggerState;
    ...
    if (objectRecognitionGateText && !objectArmBusy) {
        objectRecognitionGateText.textContent = readyForClick
            ? "Press to warm up, then present an item"
            : "Object recognition is paused";
    }
}

function maybeToastCapture(data) {
    const classes = Array.isArray(data.last_classes) ? data.last_classes.filter(Boolean) : [];
    const stamp = data.last_capture || "";
    const count = Number(data.last_detections) || 0;
    if (!stamp || count <= 0 || !classes.length) return;
    const key = `${stamp}|${classes.join(",")}|${data.mode || ""}`;
    if (key === lastCaptureToastKey) return;
    lastCaptureToastKey = key;
    const mode = String(data.mode || "").toUpperCase();
    const action = mode === "IN" ? "checked in" : mode === "OUT" ? "checked out" : "recorded";
    const label = classes.join(", ");
    const verb = classes.length === 1 ? "has" : "have";
    showCaptureAlert(true, `${label} ${verb} been ${action}`);
}
...
async function startObjectRecognition() {
    ...
    if (objectRecognitionGateText) {
        objectRecognitionGateText.textContent = "Warming up…";
    }
    try {
        ...
        applyObjectRecognitionState(true, Boolean(data.detection_armed), false, data.state || "WARMUP");
    } catch (error) {
        console.error("Object recognition start error:", error);
        applyObjectRecognitionState(true, false, false, "READY");
        if (objectRecognitionGateText) {
            objectRecognitionGateText.textContent =
                error.message || "Could not start detection";
        }
        showCaptureAlert(false, error.message || "Could not start detection");
    } finally {
        objectArmBusy = false;
        if (btnStartObjectRecognition) {
            btnStartObjectRecognition.disabled = false;
...
if (captureAlertClose) {
    captureAlertClose.addEventListener("click", hideCaptureAlert);
}
...
async function openRegisterPopup() {
    // Reset live face session (recognized person) but keep enrolled staff list.
    pendingFaceBlob = null;
    capturedForModal = false;
    lastFaceReady = false;
    lastLandmarks = null;
    currentRecognizedStaffName = "";
...
function applyFaceStatus(data) {
    ...
    if (data.recognized) {
        ...
        applyObjectRecognitionState(
            ...
            data.detection_armed ? lastTriggerState : "READY",
        );
        landmarkCtx.clearRect(0, 0, landmarkLayer.width, landmarkLayer.height);
