...
async function fetchInventory(query = "") {
    try {
        ...
        if (query) {
            url = `${CONFIG.searchUrl}?query=${encodeURIComponent(query)}`;
        }
...
function renderInventory(items) {
    ...
    if (!items.length) {
        table.innerHTML = `
            <tr class="empty-row">
                <td colspan="5">No matching items</td>
            </tr>
        `;
        return;
    }
...
cameraFeed.src = `${CONFIG.videoFeedUrl}?stream=1`;

function updateCameraStatus() {
    fetch(CONFIG.cameraStatusUrl, { cache: "no-store" })
        .then((response) => {
            if (!response.ok) {
                throw new Error("Camera status request failed");
            }
        ...
        .then((data) => {
            if (data.ok) {
                cameraStatus.textContent =
                    `CAMERA ONLINE • ${data.width} × ${data.height}`;
                cameraStatus.classList.remove("error");
            } else {
                cameraStatus.textContent =
                    "CAMERA ERROR: " + (data.error || "Unable to read camera");
...
cameraFeed.onerror = function () {
    ...
    setTimeout(() => {
        cameraFeed.src = `${CONFIG.videoFeedUrl}?stream=${Date.now()}`;
        updateCameraStatus();
